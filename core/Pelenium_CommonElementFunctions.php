 ...
 * @author Visti Kløft
 */
class Pelenium_CommonElementFunctions {
    ...
    function assertPrestate($expectedstate, $regex = false) {
        ...
        $errmsg = $this->generateErrorMessage("expectedstate != this->getCurrentPage()", "Expected prestate was '$expectedstate', but actual state was '" . $this->getCurrentPage() . "'");

        if ($regex) {

            $this->selenium->assertTrue(preg_match($expectedstate, strtolower($this->getCurrentPage())) == true, $errmsg);
        } else {
            $this->selenium->assertEquals($expectedstate, strtolower($this->getCurrentPage()), $errmsg);
        }
    }
